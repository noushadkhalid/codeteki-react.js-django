"""
Test Full Real Estate Pipeline Flow - All 8 Stages

This simulates a complete Real Estate outreach sequence:
1. Agent Found → Initial contact added
2. Invited → First invitation email sent
3. Follow Up 1 → First follow-up sent
4. Follow Up 2 → Second follow-up sent
5. Responded → Agent replied (positive)
6. Registered → Agent created account
7. Listing Properties → Agent is now listing (SUCCESS)
8. Not Interested → Alternative ending (DECLINED)

Usage:
    python manage.py test_realestate_pipeline your@email.com
    python manage.py test_realestate_pipeline your@email.com --dry-run
    python manage.py test_realestate_pipeline your@email.com --stage 3  # Only stage 3
"""
...
class Command(BaseCommand):
    help = 'Test the full Real Estate pipeline flow with all 8 stages'
    ...
    STAGES = {
        ...
        2: {
            'name': 'Invited',
            'email_type': 'agent_invitation',
            'description': 'Initial invitation email sent',
        },
        3: {
            'name': 'Follow Up 1',
            'email_type': 'agent_followup_1',
            'description': 'First follow-up (4 days after invite)',
        },
        4: {
            'name': 'Follow Up 2',
            'email_type': 'agent_followup_2',
            'description': 'Final follow-up (5 days after FU1)',
        },
        5: {
            'name': 'Responded',
            'email_type': 'agent_responded',
            'description': 'Agent responded positively',
        },
        6: {
            'name': 'Registered',
            'email_type': 'agent_registered',
            'description': 'Agent registered on platform',
        },
        7: {
            'name': 'Listing Properties',
            'email_type': 'agent_listing',
            'description': 'Agent listing properties (SUCCESS)',
            'terminal': True,
        },
        8: {
            'name': 'Not Interested',
            'email_type': None,  # No email - just archived
            'description': 'Agent declined (CLOSED)',
            'terminal': True,
        },
    }
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=str,
            default='Raj Kumar',
            help='Agent name for test contact'
        )
        parser.add_argument(
            '--company',
            type=str,
            default='Kumar Real Estate',
            help='Agency name for test contact'
        )
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"\n{'='*70}")
        self.stdout.write(f"  REAL ESTATE PIPELINE - FULL FLOW TEST {'(DRY RUN)' if dry_run else '(LIVE)'}")
        ...
        # Get Real Estate pipeline
        try:
            pipeline = Pipeline.objects.get(brand=brand, pipeline_type='realestate')
            self.stdout.write(f"✓ Pipeline: {pipeline.name}")
        except Pipeline.DoesNotExist:
            self.stdout.write(self.style.ERROR("✗ Real Estate pipeline not found"))
            return
