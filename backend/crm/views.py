...
# =============================================================================
# DEAL PROGRESSION API (for frontend/external calls)
# =============================================================================

@method_decorator(csrf_exempt, name='dispatch')
class AgentProgressionView(View):
    """
    API endpoint to progress agent deals through the Agents & Agencies pipeline.

    POST /api/crm/agent/progress/
    {
        "email": "agent@example.com",
        "action": "profile_complete"  // or agency_created, team_invited, first_listing, active_lister
    }

    POST /api/crm/agent/complete/
    {
        "email": "agent@example.com",
        "reason": "Fully onboarded with 5 listings"
    }
    """
    ...
    def post(self, request):
        ...
        from crm.services.deal_progression import progress_agent_deal, mark_agent_complete

        # If action provided, progress to that stage
        if action:
            success = progress_agent_deal(email, action)
            if success:
                return JsonResponse({
                    'success': True,
                    'message': f'Progressed {email} to {action}'
                })
            else:
                return JsonResponse({
                    'success': False,
                    'error': f'Could not progress deal for {email}. Check if deal exists and action is valid.'
                }, status=400)

        # If reason provided (no action), mark as complete
        elif reason:
            success = mark_agent_complete(email, reason)
            if success:
                return JsonResponse({
                    'success': True,
                    'message': f'Marked {email} as complete: {reason}'
                })
            else:
                return JsonResponse({
                    'success': False,
                    'error': f'Could not mark deal complete for {email}'
                }, status=400)

        else:
            return JsonResponse({
                'success': False,
                'error': 'Either action or reason is required'
            }, status=400)
